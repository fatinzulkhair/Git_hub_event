...
RULES = [
    ...
    ("VAL-008", "Location Master Data – UK Field Completeness", None, "val_logic_1_UK_field",
     "If Country starts with a UK/GB code, then UK_TPD, UI_ID and UKSLL must all exist; "
     "otherwise the rule passes automatically",
     "Critical", "Y", -1, "Missing UK_TPD/UI_ID/UKSLL for UK location"),
]
...
CONDITIONS = [
    ...
    ("VAL-003", "Target", "SGLN", None, None, "Regex", "^urn:epc:id:sgln:.+$", None, None),
...
CHECKS = [
    ...
    ("val_operator_1", "Data Type", "UK_TPD,UI_ID,UKSLL", "Exist", None, None, "AND"),
    ("val_operator_2", "Operator", "Country", "starts_with", "UK,GB", None, "AND"),
...
def seed():
    ...
    try:
        ...
        conn.commit()
        print(f"Seeded {len(RULES)} rules, {len(CONDITIONS)} conditions, "
